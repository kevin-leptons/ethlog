...
class LogRangeValve {
     ...
     */
    get value() {
        return new UInt(this._value)
    }

    /**
     *
     * @param {object} config
     * @param {UInt} [config.upperBoundary]
     * @param {UInt} [config.smallChangeStep]
     * @param {UInt} [config.bigChangeStep]
     */
    constructor(config) {
        let {upperBoundary, smallChangeStep, bigChangeStep} = config
        this._upperBoundary = upperBoundary ? upperBoundary.value : 5000
        this._value = Math.trunc(
            this._upperBoundary / 2
        )
        smallChangeStep = smallChangeStep ? smallChangeStep : new UInt(2)
        bigChangeStep = bigChangeStep ? bigChangeStep : new UInt(10)
        this._smallChangeValue = Math.trunc(
            smallChangeStep.value * this._upperBoundary / 100
        )
        this._bigChangeValue = Math.trunc(
            bigChangeStep.value * this._upperBoundary / 100
        )
        this._actions = [
...
class ChainLocker {
     ...
     * @param {BigUInt} blockNumber - Require safe block number for accessing.
    ...
    _lockTimestampFor(error, timespan) {
        ...
        console.log('lock by', ErrorString[error], 'for', timespan.value)
    }
...
class Metadata {
     ...
     *
     * @param {object} values
     * @param {BigUInt} values.latestBlockNumber
     * @param {BigUInt} values.confirmedBlockNumber
     */
    constructor(values) {
        let {latestBlockNumber, confirmedBlockNumber} = values
...
class TrackedNode {
     ...
     * @param {UInt} [config.logRangeUpperBoundary]
     * @param {UInt} [config.logRangeSmallChangeStep]
     * @param {UInt} [config.logRangeBigChangeStep]
     */
    constructor(config) {
        let {
            endpoint, quota, timeout, blockGap,
            logRangeUpperBoundary, logRangeSmallChangeStep,
            logRangeBigChangeStep
        } = config
        timeout = timeout || new UInt(3000)
        this._blockGap = blockGap || new BigUInt(15n)
        this._node = new Node({endpoint, timeout})
        let logRangeValve = new LogRangeValve({
            upperBoundary: logRangeUpperBoundary,
            smallChangeStep: logRangeSmallChangeStep,
            bigChangeStep: logRangeBigChangeStep
        })
        this._locker = new ChainLocker({quota, logRangeValve})
    }
    ...
    async getBlockByNumber(blockNumber) {
        ...
        let safeBlockNumber = latestBlockNumber.value - this._blockGap.value
     ...
     *
     * @return {Promise<Result<BigUInt, BigUInt>>}
     */
    async _getSafeBlockNumber() {
        ...
        let data = new BigUInt(safeBlockNumber)
    ...
    _getSafeLogFilter(filter, safeBlockNumber) {
        ...
        if (safeBlockNumber < toBlock) {
            toBlock = safeBlockNumber
        }
        let data = new LogFilter({
            fromBlock: filter.fromBlock,
            toBlock: new BigUInt(toBlock),
