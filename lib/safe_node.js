...
const TIMESPAN_30_SECONDS = Timespan.fromSeconds(30).open()
...
class ChainLocker {
    ...
    handleBadServer(error) {
        this._lockTimestampFor(error, TIMESPAN_30_SECONDS)
    }
    ...
    _handleNodeError(error) {
        switch (error.code) {
            ...
            case NODE_OVERLOADING:
                this._ocker.handlerOverloading(error)
                break
            case NODE_REQUEST_QUOTA:
                this._locker.handleQuotaLimit(error)
                break
            default:
