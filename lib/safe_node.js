...
class LogRangeValve {
    ...
    up(metadata) {
        ...
        if (this._lockSetRangeTo > Date.now() && newValue > this._value) {
            return
        }
        this._showMetadata(metadata)
    ...
    down() {
        ...
        this._lockSetRangeTo = Date.now() - Timespan.fromMinutes(1n).open()
    ...
    _showMetadata(metadata) {
        this._log.info('response size', metadata.responseSize.toHumanString())
        this._log.info('response time', metadata.responseTime.toHumanString())
        this._log.info('response quantity', metadata.logQuantity.value)
        this._log.info('response size', metadata.responseSize.toHumanString())
    }
...
class SafeNode {
    ...
    /**
     * @param {object} config
     * @param {EthEndpoint} config.endpoint - URL and credentials for connecting
     * to a ETH node.
     * @param {StdioLog} config.log
     */
    constructor(config) {
