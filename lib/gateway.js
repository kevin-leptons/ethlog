...
class Gateway {
    /**
     * Consider {@link Gateway.create} for initialization.
     *
     * @param {object} config
    ...
    async proxy(method, params = []) {
        for (let i = 1; i <= this._nodes.length; ++i) {
            ...
            let {data: node} = r1
            this._log.info(node.host, method)
            let r2 = await node[method](...params)
            if (r2.error) {
                this._log.warn(node.host, r2.error.message)
                continue
            }
            this._log.info(node.host, 'ok')
            return r2
        }
        if (this._lowerLayer) {
            return await this._lowerLayer.proxy(method, params)
        }
        return Result.badError(GATEWAY_BAD_BACKEND, 'no healthty nodes')
    }
