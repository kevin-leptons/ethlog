...
const ETH_ERROR_CODE_BEGIN = 0x0000ffff
const NODE_ERROR_CODE_BEGIN = 0x01000000
...
const ErrorCode = {
    ...
    NOT_OBJECT: 18,
      ...
      */
     ETH_IMPLICIT_OVERLOADING: ETH_ERROR_CODE_BEGIN + 0,
      ...
      */
     ETH_EXPLICIT_OVERLOADING: ETH_ERROR_CODE_BEGIN + 1,
      ...
      * terminate process.
      */
     ETH_BAD_REQUEST: ETH_ERROR_CODE_BEGIN + 2,
      ...
      * server is fixed.
      */
     ETH_BAD_RESPONSE: ETH_ERROR_CODE_BEGIN + 3,
      ...
      */
     ETH_BAD_SERVER: ETH_ERROR_CODE_BEGIN + 4,
     ETH_NO_BLOCK: ETH_ERROR_CODE_BEGIN + 5,
     ETH_NO_TRANSACTION: ETH_ERROR_CODE_BEGIN + 6,

     NODE_TIMESTAMP_LOCK: NODE_ERROR_CODE_BEGIN + 0,
     NODE_REQUEST_QUOTA: NODE_ERROR_CODE_BEGIN + 1,
     NODE_UNSAFE_BLOCK: NODE_ERROR_CODE_BEGIN + 2,
     NODE_BAD_SERVER_LOCK: NODE_ERROR_CODE_BEGIN + 3,
     NODE_BAD_RESPONSE_LOCK: NODE_ERROR_CODE_BEGIN + 4,
     NODE_IMPLICIT_OVERLOADING_LOCK: NODE_ERROR_CODE_BEGIN + 5,
     NODE_EXPLICIT_OVERLOADING_LOCK: NODE_ERROR_CODE_BEGIN + 6,
}
...
class Result {
     ...
     */
    constructor(
        error, data = undefined, message = undefined, metadata= undefined
    ) {
        this._error = error
        this._data = data
...
class BigUInt {
     ...
     * @returns {Result<BigUInt>}
     */
    static fromNumber(number) {
        if (isUnsignedInteger(number)) {
            let bigint = BigInt(number)
            let data = new BigUInt(bigint)
            return Result.ok(data)
        }
        if (isBigUnsignedInteger(number)) {
            let data = new BigUInt(number)
            return Result.ok(data)
        }
        return Result.error(
            ErrorCode.NOT_ACCEPTED_SIZE, 'not a number or bigint'
        )
    }
...
class BigPInt {
     ...
     *
     * @param {bigint | number} number
     * @returns {Result<BigPInt>}
     */
    static fromNumber(number) {
        if (isPositiveInteger(number)) {
            let bigint = BigInt(number)
            let data = new BigPInt(bigint)
            return Result.ok(data)
        }
        if (isBigPositiveInteger(number)) {
            let data = new BigPInt(number)
            return Result.ok(data)
        }
        return Result.error(
            ErrorCode.NOT_ACCEPTED_SIZE, 'not a number or bigint'
...
class LogTopicCombination {
    ...
    static fromRpcResult(value) {
        ...
        for (let heximal of value) {
            let {error, data: topic} = ByteData32.fromHeximal(heximal)
            if (error) {
...
/**
 * Check a value is safe, positive, integer number in Javasript.
 *
 * @private
 * @param {number} value
 * @return {boolean}
...
/**
 * Check a value is big, positive, integer number.
 *
 * @private
 * @param {bigint} value
 * @return {boolean}
 */
function isBigPositiveInteger(value) {
    return (typeof(value) === 'bigint') && (value > 0)
}
...
function validateObject(value, acceptedAttributes = []) {
    if (typeof value !== 'object' || value === null) {
        return Result.error(ErrorCode.NOT_OBJECT)
    }
