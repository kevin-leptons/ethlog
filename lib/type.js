...
const {
    Result, UInt, UInt16, UInt64, Timestamp, Timespan, DataSize, Heximal,
    validateHeximal, validateArray, validateArrayItems, validateInstance,
    validateInstanceMap, mapObject, mapArray
} = require('minitype')
...
class ByteData {
    ...
    static fromBuffer(value) {
        if ((value instanceof Buffer) === false) {
            return Result.typeError('expect a buffer')
        }
        let data = new ByteData(value)
    ...
    static fromBadHeximal(value) {
        let heximal = (value === '0x')
            ? '0x0'
            : value
        let r1 = heximalToBuffer(heximal)
        if (r1.error) {
            return r1
        }
        let data = new ByteData(r1.data)
...
class HttpEndpoint {
    ...
    /**
     * Consider {@link HttpEndpoint.create} for initialization.
     *
     * @param {object} values
     */
    constructor(values) {
...
class EthEndpoint {
    ...
    /**
     * Consider use {@link EthEndpoint.create} for initialization instead
     * of constructor.
     *
     * @param {object} values
     */
...
class EndpointQuota {
    ...
    /**
     * Consider using {@link EndpointQuota.create} for initialization.
     *
     * @param {object} config
     */
    constructor(config) {
...
class Address {
    ...
    /**
     * Consider using {@link Address.fromBuffer} or {@link Address.fromHeximal}
     * for initialization.
     *
     * @param {Buffer} value
     */
...
class Block {
    ...
    /**
     * Consider using {@link Block.fromRpcResult} for initialization.
     *
     * @param {object} config
...
class Transaction {
    ...
    /**
     * Consider using {@link Transaction.fromRpcResult} for initialization.
     *
     * @param {object} object
     */
    constructor(object) {
...
class LogTopicCombination {
    ...
    /**
     * Consider using {@link LogTopicCombination.fromRpcResult} for
     * initialization.
     *
...
class LogTopicFilter {
    ...
    /**
     * Consider using {@link LogTopicFilter.create} for initialization.
     *
     ...
     *     ByteData32.fromHeximal('0x1234f...')
     * ])
     *
     * // Specific topic[0] with nested topic filters.
     * let topcis = LogTopicFilter.create([
     *     [
     *         ByteData32.fromHeximal('0x1234...'),
     *         ByteData32.fromHeximal('0xabcdef...')
     *     ]
     * ])
     */
    static create(value = []) {
...
class LogFilter {
    ...
    /**
     * Consider using {@link LogFilter.create} for initialization.
     *
     * @param {object} values
     */
    constructor(values) {
     ...
     * @param {LogTopicFilter} [object.topics]
...
class Log {
    ...
    /**
     * Consider using {@link Log.fromRpcResult} for initialization.
     *
     * @param {object} values
