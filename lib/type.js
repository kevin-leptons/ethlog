...
class Endpoint {
    ...
    constructor(values) {
        ...
        this._logRangeBoundary = logRangeBoundary || new UInt64(5000n)
        this._logSizeBorder = logSizeBorder || new DataSize(0x400000n)
        this._logTimeBorder = logTimeBorder || new Timespan(5000n)
        this._logQuantityBorder = logQuantityBorder || new UInt64(10000n)
        this._timeout = new Timespan(this._logTimeBorder.value + 6000n)
    }

    /**
     *
     * @param {Array<Endpoint>} items
     * @param {string} hint
     * @throws {ResultError}
     */
    static assertUniqueItems(items, hint) {
        ...
        items.forEach((v, i) => {
            assertInstance(v, Endpoint, `${hint}[${i}]`)
        })
        let identities = items.map(Endpoint._getIdentity)
     ...
     *
     * @param {Array<Endpoint>} list1
     * @param {Array<Endpoint>} list2
     * @param {string} hint
     * @throws {ResultError}
     */
    static assertUniqueLists(list1, list2, hint) {
        Endpoint.assertUniqueItems(list1, hint)
        Endpoint.assertUniqueItems(list2, hint)
        let set1 = new Set(
            list1.map(Endpoint._getIdentity)
        )
        let set2 = new Set(
            list2.map(Endpoint._getIdentity)
        )
