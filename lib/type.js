...
const {
    Result,
    UInt16, UInt64, Timestamp, Timespan, DataSize,
    validateHeximal,
    mapObject, mapArray,
} = require('minitype')
...
class ByteData32 {
     ...
     *
     * @param {Heximal} value
     * @return {Result<string, ByteData32>}
     */
    static fromHeximal(value) {
...
class EthEndpoint {
    ...
    constructor(values) {
        ...
        this._logSafeGap = logSafeGap || UInt64.fromNumber(15).open()
        this._logRangeBoundary = logRangeBoundary || UInt64.fromNumber(5 * 1000).open()
        this._logSizeBorder = logSizeBorder || DataSize.fromMegabytes(4).open()
        this._logTimeBorder = logTimeBorder || Timespan.fromSeconds(5).open()
        this._logQuantityBorder = logQuantityBorder || UInt64.fromNumber(10 * 1000).open()
        this._timeout = Timespan.fromMiliseconds(this._logTimeBorder.value + 6 * 1000).open()
    }
...
class LogFilter {
    ...
    constructor(values) {
        ...
        if (fromBlock.value > toBlock.value) {
            Result.error('expect fromBlock > toBlock').open()
        }
...
function heximalToFixedBuffer(value, size) {
    ...
    if (digits.length !== (2 * size)) {
        return Result.error(`expect a heximal ${size} bytes`)
    }
