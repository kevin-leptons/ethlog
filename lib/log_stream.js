...
class LogStream {
    ...
    static _assertConstruction(config) {
        ...
        assertArray(mainEndpoints, 'mainEndpoints')
        assertOptionalArray(backupEndpoints, 'backupEndpoints')
        backupEndpoints.forEach((v, i) => {
            assertInstance(v, Endpoint, `backupEndpoints[${i}]`)
        })
        ...
        assertOptionalInstance(logLevel, LogLevel, 'logLevel')
    }
}
